...
Sports analytics has grown tremendously in recent years due to increased demand for data-driven insights from machine learning tools and advanced statistical models. Sports analytics has become a discipline of data science and can help predict the game scores and rank teams. However, in sports, the most fundamental question for teams and fans is who will win or lose. Building a binary outcome model for head-to-head games can help predict these results.
...
2.  The data should be structured to record paired comparisons, where one team wins against another. The data shouldn't contain missing values or duplicates. [@IdentifyRemoveDuplicate].
...
#Loading the hoopsno.csv file
data <- read.csv("hoopsno.csv")
```

# Converting Columns

A common error in fitting models is having incorrectly named columns, so please ensure the columns names include the winner,loser, and win_count.
...
$$ P(A \text{ beats } B) = \frac{e^{(strength A)}}{e^{(strength A)} + e^{(strength B)}} $$
$$ P(Iowa \text{ beats } Nebraska) = \frac{e^{(0)}}{e^{(0)} + e^{(-1.1)}}=0.75 $$ 
...
To learn more about advanced binary outcome model and ranking systems, such as Elo and Glickman. [@glickmanModelsRatingSystems2024].
